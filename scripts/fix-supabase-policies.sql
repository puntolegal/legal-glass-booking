-- Script CORREGIDO para configurar las políticas de Supabase
-- Maneja errores si las políticas ya existen
...
DROP POLICY IF EXISTS "Permitir leer reservas por email" ON public.reservas;
DROP POLICY IF EXISTS "Solo admins pueden eliminar reservas" ON public.reservas;

-- 3. Crear políticas NUEVAS (ahora no habrá conflictos)
-- Permitir a cualquiera crear reservas (necesario para clientes no autenticados)
CREATE POLICY "Permitir crear reservas publicas" ON public.reservas
...
-- 4. Eliminar función existente si existe
DROP FUNCTION IF EXISTS public.create_public_reservation;

-- 5. Crear función para insertar reservas sin autenticación
...
-- 6. Dar permisos para ejecutar la función
...
SELECT 
    'Tabla reservas configurada correctamente' as mensaje,
    schemaname,
    tablename,
    rowsecurity
FROM pg_tables 
...
WHERE routine_name = 'create_public_reservation';

-- Mensaje final
SELECT '✅ CONFIGURACIÓN COMPLETADA EXITOSAMENTE - Tu sistema está listo para recibir reservas' as resultado_final;
