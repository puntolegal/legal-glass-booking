...
console.log('🔄 LIMPIEZA CON NUEVOS IDs\n');
...
async function cleanWithNewIds() {
  try {
    ...
    // Preparar reservas únicas con nuevos IDs
    const newReservations = uniqueList.map(reservation => ({
      ...reservation,
      id: randomUUID(), // Generar nuevo ID único
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString()
    }));
    
    // Recrear solo las reservas únicas
    console.log('\n🔄 Recreando reservas únicas con nuevos IDs...');
    ...
    console.log(`✅ ${insertedReservations.length} reservas únicas recreadas`);
    
    // Mostrar resultado final
    console.log('\n📊 RESULTADO FINAL:');
    console.log(`   Reservas únicas: ${insertedReservations.length}`);
    console.log(`   Duplicados eliminados: ${allReservations.length - insertedReservations.length}`);
    
    console.log('\n📋 Reservas finales:');
    insertedReservations.forEach((reservation, index) => {
