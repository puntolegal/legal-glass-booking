...
/** Alinear con `src/config/siteUrl.ts` (host canónico sin www). */
const SITE_ORIGIN = "https://puntolegal.online";
...
export class MercadoPagoAPI {
  ...
  async createPreference(paymentData: PaymentData): Promise<MercadoPagoPreference> {
    const preference = {
      ...
      back_urls: {
        success: process.env.SUCCESS_URL || `${SITE_ORIGIN}/payment-success?source=mercadopago`,
        failure: process.env.FAILURE_URL || `${SITE_ORIGIN}/payment-failure?source=mercadopago`,
        pending: process.env.PENDING_URL || `${SITE_ORIGIN}/payment-pending?source=mercadopago`,
      },
